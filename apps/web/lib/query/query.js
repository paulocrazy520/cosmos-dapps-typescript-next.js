...
const QueryClient = async (chainId, restUrls, opts) => {
  ...
  const restUrl = await findAvailableUrl(restUrls, "rest", {
  ...
  const getCommission = (validatorAddress, opts) => {
    return axios
      .get(
        `${restUrl}/cosmos/distribution/v1beta1/validators/${validatorAddress}/commission`,
        opts
      )
      .then((res) => res.data)
      .then((result) => {
  ...
  const getProposals = (opts) => {
    ...
    return getAllPages((nextKey) => {
      ...
      return axios
        .get(
          restUrl + "/cosmos/gov/v1beta1/proposals?" + searchParams.toString(),
          opts
        )
        .then((res) => res.data);
    }).then((pages) => {
  ...
  const getProposalTally = (proposal_id, opts) => {
    return axios
      .get(
        restUrl + "/cosmos/gov/v1beta1/proposals/" + proposal_id + "/tally",
        opts
      )
      .then((res) => res.data);
  };
  ...
  const getGranteeGrants = (grantee, opts, pageCallback) => {
    ...
    return getAllPages((nextKey) => {
      ...
      return axios
        .get(
          restUrl +
            "/cosmos/authz/v1beta1/grants/grantee/" +
            grantee +
            "?" +
            searchParams.toString(),
          opts
  ...
  async function getLatestBlock(url, type, path, opts) {
    const { timeout } = opts || {};
    try {
      return await axios.get(url + path, { timeout }).then((res) => res.data);
    } catch (error) {
