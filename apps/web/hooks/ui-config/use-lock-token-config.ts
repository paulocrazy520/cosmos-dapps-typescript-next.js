...
export function useLockTokenConfig(sendCurrency?: AppCurrency | undefined): {
  ...
  const unlockTokens = useCallback(
      ...
      return new Promise<"synthetic" | "normal">(async (resolve, reject) => {
        try {
          ...
          const locks = blockGasLimitLockIds.map((lockId) => ({
            lockId,
            isSyntheticLock:
              queryOllo.querySyntheticLockupsByLockId.get(lockId)
                .isSyntheticLock === true,
          }));

          const durations = queryOllo.queryLockableDurations.lockableDurations;
    ...
    [
      queryOllo,
      queryOllo.querySyntheticLockupsByLockId,
      queryOllo.queryLockableDurations.response,
    ]
  );
  ...
  useEffect(() => {
    if (
      queryOllo.queryAccountLocked.get(bech32Address).isFetching ||
      bech32Address === ""
    )
      return;

    const unlockingTokens =
      queryOllo.queryAccountLocked.get(bech32Address).unlockingCoins;
    const now = dayjs().utc();
    ...
    unlockingTokens.forEach(({ endTime }) => {
      ...
      timeoutIds.push(
        setTimeout(() => {
          // queryOllo.queryGammPoolShare.fetch(bech32Address);
        }, diffMs + blockTime)
      );
    });
  ...
  }, [queryOllo.queryAccountLocked.get(bech32Address).response, bech32Address]);
