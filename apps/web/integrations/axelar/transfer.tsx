...
import { basicIbcTransfer } from "@ollo/stores";
import {
  useFakeFeeConfig,
  useAmountConfig,
...
> = observer(
  ({
    ...
    const { bech32Address } = olloAccount;
    const originCurrency = balanceOnollo.balance.currency.originCurrency!;
    ...
    // WITHDRAWING: is an IBC transfer ollo->Axelar
    const feeConfig = useFakeFeeConfig(
      chainStore,
      chainId,
      olloAccount.cosmos.msgOpts.ibcTransfer.gas
    );
    const withdrawAmountConfig = useAmountConfig(
      ...
      bech32Address,
      feeConfig,
      balanceOnollo.balance.currency
    );

    // chain path info whether withdrawing or depositing
    const olloPath = {
      address: bech32Address,
      networkName: chainStore.ollo.chainName,
    ...
    const sourceChain = isWithdraw ? "ollo" : selectedSourceChainAxelarKey;
    const destChain = isWithdraw ? selectedSourceChainAxelarKey : "ollo";
        ...
        if (amount !== "") {
          nonIbcBridgeHistoryStore.pushTxNow(
            ...
            new CoinPretty(
              ...
              .toString(),
            isWithdraw,
            olloAccount.bech32Address // use ollo account for account keys (vs any EVM account)
          );
        }
      },
    ...
    const doAxelarTransfer = useCallback(async () => {
            ...
            await basicIbcTransfer(
              {
                account: olloAccount,
                chainId,
                channelId: balanceOnollo.sourceChannelId,
              },
              {
                account: depositAddress,
                chainId: axelarChainId,
                channelId: balanceOnollo.destChannelId,
              },
      ...
      axelarChainId,
      chainId,
      balanceOnollo.sourceChannelId,
      balanceOnollo.destChannelId,
      depositAddress,
      depositAmount,
      erc20ContractAddress,
      ethWalletClient,
    ...
    const userCanInteract =
      (!isWithdraw &&
        ...
        !isEthTxPending) ||
      (isWithdraw && olloAccount.txTypeInProgress === "");
    ...
    return (
      <>
        {/* <Transfer
          ...
          transferPath={[
            isWithdraw ? olloPath : counterpartyPath,
            {
              bridgeName: "Axelar",
              bridgeIconUrl: "/icons/axelar.svg",
              isLoading: isDepositAddressLoading,
            },
            isWithdraw ? counterpartyPath : olloPath,
          ]}
