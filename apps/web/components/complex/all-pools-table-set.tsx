...
import { ObservablePoolWithFeeMetrics } from "@ollo/stores";
...
export const AllPoolsTableSet: FunctionComponent<{
  ...
  const queriesOllo = queriesStore.get(chainId).ollo!;
  const queriesExternal = queriesExternalStore.get();
  const account = accountStore.getAccount(chainId);
  const fiat = priceStore.getFiatCurrency(priceStore.defaultVsCurrency)!;

  const allPools = queriesOllo.queryGammPools.getAllPools();
    ...
    () =>
      allPools.map((pool) => ({
        ...
        myLiquidity: pool
          ...
          .mul(
            queriesOllo.queryGammPoolShare.getAllGammShareRatio(
              account.bech32Address,
              pool.id
            )
          ),
        apr: queriesOllo.queryIncentivizedPools
    ...
    () =>
      allData.map((poolWithFeeMetrics) => ({
        ...
        onClick: () => {
          ...
          //       .join(" / "),
          //     isSuperfluidPool:
          //       queriesOllo.querySuperfluidPools.isSuperfluidPool(
        ...
        return [
          ...
          },
          {
            ...
            isLoading: isIncentivizedPools
              ? queriesOllo.queryIncentivizedPools.isAprFetching
              : false,
          },
        ];
      }),
    ...
    return (
      <CompactPoolTableDisplay
        ...
        pools={allData.map((poolData) => ({
          ...
          isSuperfluidPool: queriesOllo.querySuperfluidPools.isSuperfluidPool(
            poolData.pool.id
          ),
        }))}
        searchBoxProps={{
