...
export const MyPoolCard: FunctionComponent<{
  ...
  const chainInfo = chainStore.ollo;
  const queryOllo = queriesStore.get(chainInfo.chainId).ollo!;
  ...
  const apr = queryOllo.queryIncentivizedPools.computeMostAPY(
    pool.id,
    priceStore
  );

  const shareRatio = queryOllo.queryGammPoolShare.getAllGammShareRatio(
    account.bech32Address,
    pool.id
  );
  const lockedShareRatio = queryOllo.queryGammPoolShare.getLockedGammShareRatio(
    account.bech32Address,
    pool.id
  );
