...
import { OlloQueries, IPriceStore } from "@ollo/stores";
...
export class ObservableAssets {
  ...
  get lockedCoins(): CoinPretty[] {
    return (
      this.queries.ollo?.queryLockedCoins.get(this.account.bech32Address)
        .lockedCoins ?? []
    );
  }
  ...
  public calcValueOf = computedFn((balances: CoinPretty[]): PricePretty => {
    ...
    for (const balance of balances) {
      if (balance.currency.coinMinimalDenom.startsWith("gamm/pool/")) {
        ...
        const pool = this.queries.ollo?.queryGammPools.getPool(poolId);
        if (pool) {
