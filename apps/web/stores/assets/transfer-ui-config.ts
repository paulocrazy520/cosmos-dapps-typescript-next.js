...
//       balanceOnOllo.originBridgeInfo!.wallets.includes(key)
//     );
//     const alreadyConnectedWallet = applicableWallets.find(
//       (wallet) => wallet.isConnected
//     );
...
//         ? []
//         : balanceOnOllo.fiatRamps?.map(({ rampKey }) => rampKey),
//       onSelectSource: (key) => {
//         const selectedWallet = wallets.find((wallet) => wallet.key === key);
//         const selectedFiatRamp = balanceOnOllo.fiatRamps?.find(
//           ({ rampKey }) => rampKey === key
//         );
...
//     onRequestBack?: () => void
//   ) {
//     if (!balanceOnOllo.originBridgeInfo) {
